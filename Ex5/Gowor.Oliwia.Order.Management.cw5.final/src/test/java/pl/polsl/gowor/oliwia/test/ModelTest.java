...
import entities.Product.VatRate;
...
public class ModelTest {
    ...
    public void setUp() {
        ...
        String buyer = "name surname address";
        Order order1 = new Order("1", new Date(1, 1, 1), buyer, "card");
        order1.setProducts(products);
        Order order2 = new Order("2", new Date(2, 2, 2), buyer, "card");
    ...
    /**
     * Test checks if method addOrder works properly when number order's number
     * is repeated.
     *
    ...
    /**
     * Test checks if method addOrder works properly when the data is correct.
     *
     * @param order parameter represents order to be added to the list
     */
    @ParameterizedTest
    ...
    public void shouldAddCorrectOrder(Order order) {
        ...
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getNumber(), newOrder.getNumber(), "Number should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getBuyer(), newOrder.getBuyer(), "Buyers data should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getOrderDate().getDay(), newOrder.getOrderDate().getDay(), "Day should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getOrderDate().getMonth(), newOrder.getOrderDate().getMonth(), "Month should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getOrderDate().getYear(), newOrder.getOrderDate().getYear(), "Year should be the same in both instances");
        for (int i = 0; i < model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().size(); i++) {
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getProductName(), newOrder.getProducts().get(i).getProductName(), "Product name should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getPriceNetto(), newOrder.getProducts().get(i).getPriceNetto(), "Product Netto price should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getValueNetto(), newOrder.getProducts().get(i).getValueNetto(), "Product Netto value should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getValueBrutto(), newOrder.getProducts().get(i).getValueBrutto(), "Product Brutto value should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getValueVat(), newOrder.getProducts().get(i).getValueVat(), "Product VAT value should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getVatRate(), newOrder.getProducts().get(i).getVatRate(), "Product VAT rate should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getQuantinity(), newOrder.getProducts().get(i).getQuantinity(), "Product quantinity value should be the same in both instances");
            assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getProducts().get(i).getUnit(), newOrder.getProducts().get(i).getUnit(), "Product unit should be the same in both instances");
        }
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getPaymentMethod(), newOrder.getPaymentMethod(), "Payment method should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getSumNetto(), newOrder.getSumNetto(), "Sum Netto should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getSumBrutto(), newOrder.getSumBrutto(), "Sum Brutto should be the same in both instances");
        assertEquals(model.getOrdersList().get(model.getOrdersList().size() - 1).getSumVat(), newOrder.getSumVat(), "Sum VAT should be the same in both instances");
    }
    ...
    public void shouldThrowExceptionWhenOrderNumberToFindIsNotOnListOrNull(String number) {
        try {
            model.searchOrderByNumber(number);
            fail("An exception should be thrown when the order is not on the list");
        } catch (OrderNotFoundException ex) {
        }
    }
    ...
    public void shouldAddNewProduct(Product product) {
        ...
        Order order = (Order) model.getOrdersList().get(model.getOrdersList().size() - 1);
...
// -------------------------------------------------------------
    ...
    static Stream dataOrdersRepeatedNumbers() {
        ...
        order1.setProducts(products);
        Order order2 = new Order("2", new Date(2, 2, 2), "name surname address", "card");
        order2.setProducts(products);
    ...
    static Stream dataOrdersRightNumbers() {
        List<Product> products = Arrays.asList(new Product("name", 10, VatRate.TWENTYTHREE, 10, "szt"));
        Order order1 = new Order("new-number", new Date(1, 1, 1), "name surname address", "card");
        order1.setProducts(products);
        Order order2 = new Order("not-repeated", new Date(2, 2, 2), "name surname address", "card");
        order2.setProducts(products);
        return Stream.of(order1, order2);
    }
